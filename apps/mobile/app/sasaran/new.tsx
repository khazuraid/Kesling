...
export default function NewSasaranScreen() {
  ...
  const [subCategoryId, setSubCategoryId] = useState<number | null>(sub ? Number(sub) : null);
  ...
  const mutation = useMutation({
    mutationFn: () => api.createSasaran({ nama, alamat, pemilik, kontak, subCategoryId: subCategoryId! }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sasaran"] });
      qc.invalidateQueries({ queryKey: ["dashboard"] });
      Alert.alert("Berhasil", "Sasaran ditambahkan.");
      router.back();
    },
  ...
  const submit = () => {
    ...
    if (!subCategoryId) return Alert.alert("Perhatian", "Pilih sub-kategori.");
    mutation.mutate();
  };
  ...
  return (
    <Screen>
      <PageHeader title="Tambah Sasaran" back />
      ...
      <YStack px="$4" gap="$3" mt="$2">
        ...
        <FieldInput label="Kontak" value={kontak} onChange={setKontak} placeholder="No HP / telepon" />
