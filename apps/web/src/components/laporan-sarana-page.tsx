    ...
    setFormItems(
      jenisList.map((j: any) => {
        ...
        return {
          ...
          kk: ex?.kk || 0,
          pddk: ex?.pddk || 0,
          diperiksaJumlah: ex?.diperiksaJumlah || 0,
          diperiksaMs: ex?.diperiksaMs || 0,
          diperiksaKk: ex?.diperiksaKk || 0,
          diperiksaPddk: ex?.diperiksaPddk || 0,
        };
      }),
    );
    setShowForm(true);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
            {puskesmasList.length} Puskesmas • {data.length} data
          </p>
        </div>
      ...
      <div className="card p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <PdfExportButton
            ...
            rows={grouped.map((pkm: any, i: number) => [
              i + 1,
              pkm.nama,
              ...pkm.items.flatMap((item: any) => FIELDS.map((f) => item?.[f] || 0)),
            ])}
          ...
          <table className="w-full text-xs whitespace-nowrap min-w-[800px]">
            <thead>
              <tr className="border-b border-[hsl(var(--border))]">
                <th className="px-3 py-3 text-left text-[10px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider sticky left-0 bg-[hsl(var(--card))] z-10">
                  No
                </th>
                <th className="px-3 py-3 text-left text-[10px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider sticky left-8 bg-[hsl(var(--card))] z-10">
                  Puskesmas
                </th>
                {jenisList.map((j: any) => (
                  <th
                    key={j.id}
                    colSpan={7}
                    className="px-2 py-3 text-center text-[10px] font-semibold text-[hsl(var(--primary))] uppercase tracking-wider border-l border-[hsl(var(--border))]"
                  >
                    {j.nama}
                  </th>
                ))}
                ...
                {jenisList.map((j: any) => (
                  <Fragment key={j.id}>
                    {HEADERS.map((h, i) => (
                      <th
                        key={i}
                        className={`px-2 py-2 text-center text-[10px] text-[hsl(var(--muted-foreground))] ${i === 0 ? "border-l border-[hsl(var(--border))]" : ""}`}
                      >
                        {h}
                      </th>
                    ))}
            ...
            <tbody>
              {isLoading
                ? Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i} className="border-b border-[hsl(var(--border))]">
                      <td colSpan={2 + jenisList.length * 7} className="px-4 py-3">
                        <div className="h-4 bg-[hsl(var(--muted))]animate-pulse" />
                      </td>
                    </tr>
                  ))
                : grouped.map((pkm: any, i: number) => (
                    ...
                    >
                      <td className="px-3 py-2.5 sticky left-0 bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))]">
                        {i + 1}
                      </td>
                      ...
                      {pkm.items.map((item: any, idx: number) => (
                        <Fragment key={idx}>
                          {FIELDS.map((f, fi) => (
                            <td
                              key={f}
                              className={`px-2 py-2.5 text-center ${fi === 0 ? "border-l border-[hsl(var(--border))]" : ""} ${item?.[f] ? "" : "text-[hsl(var(--muted-foreground))]/40"}`}
                            >
                              {item?.[f] || "—"}
                            </td>
                          ))}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div
            className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[85vh] overflow-y-auto"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-[hsl(220,13%,82%)]">
                      ...
                      {HEADERS.map((h) => (
                        <th key={h} className="px-2 py-2.5 text-center text-xs font-semibold text-slate-600 w-16">
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    {formItems.map((item, idx) => (
                      <tr key={item.jenisSaranaId} className="border-t border-slate-100">
                        <td className="px-3 py-2 text-xs font-medium text-slate-700">
                          {jenisList.find((j: any) => j.id === item.jenisSaranaId)?.nama}
                        </td>
                        {FIELDS.map((f) => (
