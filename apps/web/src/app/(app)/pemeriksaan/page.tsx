...
import { ArrowLeft, Check, Clock, Edit3, FileText, MapPin, Trash2, X, Plus, ShieldAlert } from "lucide-react";
...
export default function PemeriksaanPage() {
  ...
  function handleSubmit() {
    ...
    const valuesArray = Object.entries(formValues).map(([fid, val]) => {
      ...
      return {
        ...
        valueNumber: field?.tipe === "NUMBER" && val !== null && val !== undefined && val !== "" ? parseFloat(val) || 0 : null,
      };
    });
    ...
    if (tanggalPemeriksaan) {
      ...
      if (!isNaN(d.getTime())) { bulan = d.getMonth() + 1; tahun = d.getFullYear(); }
    }
  ...
  const metaFields = useMemo(() => activeForm?.fields.filter((f: any) => f.grup === "__META__") || [], [activeForm]);
  const fieldsToChecklist = useMemo(() => activeForm?.fields.filter((f: any) => f.grup !== "__META__") || [], [activeForm]);
  ...
  const liveSkor = useMemo(() => {
    ...
    let gained = 0; let max = 0; let countBenar = 0; let countSalah = 0; const countTotal = penilaian.length;
    for (const f of penilaian) {
      ...
      if (f.tipe === "BOOLEAN") {
        if (val === "TRUE") { gained += skorBenar; countBenar++; }
        else if (val === "FALSE") { gained += skorSalah; countSalah++; }
      } else if (f.tipe === "NUMBER") { gained += (parseFloat(val) || 0) * (f.config?.skor ?? 0); }
      else if (val) { gained += (f.config?.skor ?? 0); }
    }
    let finalValue = gained; let valueText = String(Math.round(gained));
    if (rumus === "percentage" || rumus === "weighted") {
      ...
      try {
        ...
        if (!isNaN(res)) { finalValue = res; valueText = String(Math.round(finalValue * 10) / 10); }
      } catch {}
    }
    let pass = false;
  ...
  const canSubmit = activeForm &&
    fieldsToChecklist.filter((f: any) => f.isRequired !== false).every((f: any) => {
      const val = formValues[f.id];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-3">
        ...
        <p className="text-[11px] text-[hsl(var(--muted-foreground))]">
          Halaman ini hanya untuk Operator / Puskesmas
        </p>
      </div>
    );
  }
      ...
      <div className="w-full mx-auto pb-4 space-y-4 fade-in">
        ...
        {activeTab === "formulir" && (
          <div className="border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
            {templates.length === 0 ? (
              <div className="p-8 text-center"><p className="text-[12px] font-bold">Belum ada template</p></div>
            ) : (
              <div className="divide-y divide-[hsl(var(--border))]">
                {templates.map((tpl) => (
                  <div key={tpl.id} className="flex items-center justify-between p-4 hover:bg-[hsl(var(--muted))]/30 transition-colors">
                    <div>
                      ...
                      <p className="text-[11px] text-[hsl(var(--muted-foreground))] mt-0.5">
                        {tpl.deskripsi || "-"} • {tpl.fields?.filter((f: any) => f.grup !== "__META__").length || 0} Variabel
                      </p>
                    </div>
                    <button onClick={() => handleOpenForm(tpl)} className="px-4 py-1.5 border border-[hsl(var(--border))] bg-[hsl(var(--background))] text-[11px] font-bold uppercase hover:bg-[hsl(var(--foreground))] hover:text-[hsl(var(--background))] transition-colors">
                      Buka Form
                    </button>
                  </div>
                ))}
        ...
        {activeTab === "riwayat" && (
          <div className="border border-[hsl(var(--border))] bg-[hsl(var(--card))] overflow-x-auto">
            {results.length === 0 ? (
              <div className="p-8 text-center"><p className="text-[12px] font-bold">Belum ada riwayat</p></div>
            ) : (
                ...
                <tbody className="divide-y divide-[hsl(var(--border))]/50 text-[12px]">
                  {results.map((res) => (
                    <tr key={res.id} className="hover:bg-[hsl(var(--muted))]/30 transition-colors">
                      <td className="px-4 py-2.5">{new Date(res.createdAt).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</td>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <button onClick={() => handleEditResult(res)} className="text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]"><Edit3 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => { if (confirm(`Hapus?`)) deleteMut.mutate(res.id); }} className="text-[hsl(var(--muted-foreground))] hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
  ...
  // ================= FORM CHECKLIST VIEW =================
    return (
      ...
      <div className="px-5 pt-4 pb-2 bg-[hsl(var(--background))] border-b border-[hsl(var(--border))]">
        <label className="block text-[10px] font-black uppercase text-[hsl(var(--muted-foreground))] mb-1">Pilih Data Dasar (Sasaran)</label>
        <select 
          value={sasaranId || ""}
        ...
        <p className="text-[10px] text-[hsl(var(--muted-foreground))] mt-1">Atau isi info manual di form meta bawah ini jika belum terdaftar.</p>
      </div>
      ...
      <FormView
        ...
        onBack={() => { setActiveForm(null); setEditingId(null); setFormValues({}); setMetaValues({}); }}
