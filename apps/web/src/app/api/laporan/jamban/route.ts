...
import { laporanSaranaSchema, validateBody } from "@/lib/validations";
...
export async function GET(req: Request) {
  ...
  const data = await prisma.laporanJamban.findMany({
    where,
...
export async function POST(req: Request) {
  ...
  const parsed = validateBody(laporanSaranaSchema, raw);
