...
import { laporanSabSchema, validateBody } from "@/lib/validations";
...
export async function POST(req: Request) {
  ...
  const parsed = validateBody(laporanSabSchema, raw);
  ...
  try {
    ...
    for (const r of results) {
      await createAuditLog({ userId: user.id, action: "CREATE", tableName: "laporan_sab", recordId: r.id, newData: r });
    }
